...
function moveLeft() {
  ...
  const newPosition = Math.max(currentPosition - 2, 0); // Adjust movement speed as needed
...
function moveRight() {
  ...
  const currentPosition = parseInt(player.style.left) || 0;
  const newPosition = Math.min(currentPosition + 2, gameContainerWidth - playerWidth); // Adjust movement speed as needed
  player.style.left = newPosition + 'px';
